...
`Portal` uses CREATE2 for deterministic token deployment. This skill only launches Flap Tax
Token V3 (`TOKEN_TAXED_V3`) tokens, whose address must end in the 4-character suffix `7777`.
...
function findVanityTokenSalt(
  suffix: string,           // "7777" for TOKEN_TAXED_V3
