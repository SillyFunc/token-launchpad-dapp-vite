...
| 四通道 TaxProcessor / Dividend / BuybackVault 及工厂 | **当前这次主网部署尚未包含**。接口见测试网文档；地址必须等新一次 `Deploy.s.sol` 广播并链上核验后再写入，禁止手抄。本表现有地址仍是旧 ABI/旧行为，不得绑定当前 13 字段 `TokenConfig` ABI。 |
